...
const Sidebar = () => {
	...
	const handleDelete = (id) => {
	  ...
	  const handleOpenSavedRecipe = (recipe) => {
		...
		localStorage.setItem("isSavedRecipeView", "true");
	};
	...
	return (
		<div className={`fixed md:static top-0 left-0 z-20 h-full overflow-y-auto scrollbar-hide bg-gray-100 dark:bg-gray-950 md:w-72 w-64 placeholder: p-4 transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
				...
				{saved.map((item, id) => (
				  <div key={id} onClick={() => (handleOpenSavedRecipe(item))} className={`cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 mb-2 p-2 rounded ${selectedRecipe?.id === item.id ? 'bg-gray-300 hover:bg-gray-200 dark:bg-gray-500' : 'dark:hover:bg-gray-700'}`}>
					{item.title}
